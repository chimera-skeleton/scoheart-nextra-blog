...
- 同步加载，必须要根据路径加载完文件且执行完毕文件中的代码才完成导入，再执行后面的代码。在服务端nodejs环境中从磁盘读取文件速度快，但是如果在浏览器环境中是不适用的，因为浏览器需要发起请求，从服务器中请求js文件，而如果这种大量的请求操作都是同步执行的话，会严重影响网页交互体验。所以需要异步的方式。

---
>***那么Browser Runtime才是主流的JavaScript Runtime，所以社区形成了一些可行的针对于Browser Runtime的模块化方案，他们都遵循下面两点***
- 异步加载
- 模块代码直接放在函数中
